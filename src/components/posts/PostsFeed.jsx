...
function PostsFeed(props) {
  ...
  const hour = getHoursDiffBetweenDates(new Date(props.createdDate),new Date("2023-01-27 10:25:00"));
  ...
  return (
    ...
    >
      ...
      <Meta 
      title={postCreator}
      description={ (hour < 24 && hour >= 1) ? `${hour}h` : `${props.createdDate.slice(0,10)}`}
      style={{boxSizing:"border-box"}}
      />
        <MoreVertIcon/>
      </div>
      <div 
      style={{height:"fit-content", border:"1px solid red",justifySelf:"center",marginTop:"1em",maxHeight:"25em"}}>
         {props.image !=null ? <img src={`data:image/png;base64,${props.image}`} alt="image"/> : <h2 style={{textAlign:"center"}}>{props.description}</h2> }
      </div>
